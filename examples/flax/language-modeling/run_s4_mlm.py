...
from academic_bert_dataset_cloud.pretraining_dataset import PreTrainingDataset
...
class TrainingArguments:
    ...
    s4_learning_rate: float = field(default=1e-3, metadata={"help": "Learning rate of s4."})
    s4_adafactor: bool = field(default=False, metadata={"help": "Whether or not to replace AdamW by Adafactor for S4 parameters."})
    s4_warmup_steps: int = field(default=0, metadata={"help": "s4 warmup steps."})
    s4_num_total_train_steps: Optional[int] = field(
        default=200000,
        metadata={"help": "Maximum number of training steps of effective batch size to complete."},
    )
    s4_schedule_type: str = field(
        default='linear',
        metadata={"help": "Type of scheduler"}
    )
...
def create_learning_rate_fn(
    ...
    if schedule_type == 'constant':
        ...
        decay_fn = optax.cosine_decay_schedule(
            init_value=learning_rate, decay_steps=num_total_train_steps - num_warmup_steps, alpha=0
        )
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        # for 24hbert, they fix the tokenzier to bert-large-uncased
        "bert-large-uncased",
        do_lower_case=True,
    ...
    s4_linear_decay_lr_schedule_fn = create_learning_rate_fn(
        training_args.s4_num_total_train_steps,
        training_args.s4_warmup_steps,
        training_args.s4_learning_rate,
        training_args.s4_schedule_type,
    )
    ...
    if training_args.s4_adafactor:
        s4_optimizer = optax.adafactor(
            learning_rate=s4_linear_decay_lr_schedule_fn,
        )
    else:
        s4_optimizer = optax.adamw(
            learning_rate=s4_linear_decay_lr_schedule_fn,
            b1=training_args.adam_beta1,
            b2=training_args.adam_beta2,
            eps=training_args.adam_epsilon,
    ...
    # for s4, we use Optax.multi_transform
    s4_fn = map_nested_fn(
        lambda k, _: "s4"
        if k in ["B", "Ct", "D", "log_step", "Vc", "p", "q", "Lambda", "W"]
        # if k in ["W", "D", "log_step", "Lambda"]
    ...
    def train_step(state, batch, dropout_rng):
        ...
        metrics = jax.lax.pmean(
            {"loss": loss, "learning_rate": linear_decay_lr_schedule_fn(state.step), "s4_learning_rate": s4_linear_decay_lr_schedule_fn(state.step)}, axis_name="batch"
        )
    ...
    wandb.init(project="s4_pretrain", entity="junxiong12")

    epochs = tqdm(range(training_args.num_train_epochs), desc=f"Epoch ... (1/{training_args.num_train_epochs})",
    ...
    for epoch in epochs:
        ...
        for batch_index_number, batch_index in enumerate(tqdm(train_dataset_iterator, desc="Training...", position=1)):
            ...
            train_input_ids, train_token_type_ids, labels = pretrain_dataset_provider.get_batch(batch_index)
            ...
            if cur_step % training_args.logging_steps == 0:
                ...
                epochs.write(
                    f"Step... ({cur_step} | Process Data Samples: {total_data_sample} | Train Loss: {train_metric['loss']}, Learning Rate: {train_metric['learning_rate']}, SamplesPerSec: {total_data_sample / (time.time() - before_train_time)})"
                )
                ...
                wandb.log(
                    {
                        ...
                        'S4 Learning Rate': train_metric['s4_learning_rate'],
            ...
            if cur_step % training_args.save_steps == 0 and cur_step > 0:
                ...
                # delete checkpoints if it exceeds the limit
        ...
        if (epoch + 1) % training_args.eval_steps == 0:
            ...
            for val_batch_index_number, val_batch_index in enumerate(
                ...
                test_input_ids, test_token_type_ids, labels = validation_dataset_provider.get_batch(val_batch_index)
            ...
            # Update progress bar
            epochs.desc = f"Step... ({cur_step} | Validation Loss: {eval_metrics['loss']}, Validation Acc: {eval_metrics['accuracy']})"
    ...
    if training_args.do_eval:
        ...
        for batch_index_number, batch_index in enumerate(
                tqdm(validation_dataset_iterator, desc="Evaluating ...", position=2, smoothing=1)):
            test_input_ids, test_token_type_ids, labels = validation_dataset_provider.get_batch(batch_index)
        ...
        wandb.log(
            {
                ...
                "Validation Accuracy": eval_metrics['eval_accuracy'],
                "Validation Perplexity": eval_metrics['eval_perplexity'],
            }
            , step=cur_step
        )
